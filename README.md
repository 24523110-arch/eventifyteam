...
| Role | Pages |
|---|---|
| **Manager** | Dashboard, Finance, **Reports** (Concert Report Dashboard: review semua laporan role lain, AI generate, approve + tanda tangan digital, ekspor PDF/Word), User Management |
| **Admin / Event Organizer** | Dashboard, **Concert Schedule** (multi-event CRUD + Mulai Live/Akhiri Konser), **Report Management** (input seluruh laporan konser: kepanitiaan, pelaksanaan, dokumentasi, keuangan, tiket, evaluasi, kehadiran, media, sponsor, vendor) |
| **Security Team** | Dashboard, Live Monitoring, **Crowd Monitoring** (manual attendance input + panel Laporan Keamanan & Statistik), **Incident Center** (track/assign/resolve + panel Laporan Insiden) |

**Manual reporting (MIS, not simulated):** every report figure is entered by the role that owns the data. The Admin/EO fills the active concert's collaborative report on **Report Management** — finance (pemasukan/pengeluaran with server-side totals & laba/rugi), ticket recap (sold/used/no-show/occupancy computed automatically), plus committee, timeline, documentation, evaluations, attendance, media, sponsors, and vendors. Audience attendance on the operational dashboard is still the Security Team's headcount entry on Crowd Monitoring. **Crowd-zone occupancy is a static snapshot in the database, not a live sensor feed** — nothing in the app changes unless a person changes it. (An optional random-walk simulator still exists for demos/stress tests but is off by default; enable with `SIMULATOR_ENABLED=true`.)

**Reporting flow → AI:** once all 14 report sections are Submitted, the Manager clicks **Generate LPJ with AI** on Reports — the AI writes only the framing chapters (kata pengantar, pendahuluan, profil kegiatan, penutup) around the roles' own data, never altering the submitted figures (Claude API if `ANTHROPIC_API_KEY` is set, deterministic local generator otherwise).
...
- **Report Management (kolaboratif, satu laporan per konser aktif)**: Admin/EO membuat & mengisi 11 bagian laporan di halaman **Report Management** (menggantikan Vendor Management dan Reports & Ticket Sales); Security mengisi 3 bagian keamanan lewat panel yang tertanam di **Crowd Monitoring** (sumber daya + statistik keamanan) dan **Incident Center** (laporan insiden), lalu menekan **Submit Security Report** → status **Waiting Manager Review**. Semua kiriman masuk ke halaman **Reports** milik Manager: review per bagian (bisa dikembalikan dengan catatan revisi), **Generate LPJ with AI** (Claude API, fallback lokal — data isian role tidak pernah diubah AI), edit narasi, setujui dengan tanda tangan digital + tanggal, ekspor PDF / Word (.docx) / print, dan bagikan via email/link.
